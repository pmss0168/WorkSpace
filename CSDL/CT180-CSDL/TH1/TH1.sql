...
create table KHOAHOC (
  ...
  NGAYBD DATE,
  NGAYKT DATE,
  CONSTRAINT KH_CHECK_NGAY CHECK (NGAYKT>NGAYBD)
);
...
CREATE TABLE LOP (
  ...
  SISO INT NOT NULL, 
  MAKH CHAR(4) NOT NULL REFERENCES KHOAHOC (MAKH),
  CONSTRAINT LOP_CHECK_SISO CHECK (SISO>12)
);
CREATE TABLE HOCVIEN (
  ...
  CONSTRAINT HV_CHECK_GT CHECK ((GIOITINH = 0) OR (GIOITINH = 1))
);
CREATE TABLE PHIEUTHU(
  ...
  CONSTRAINT PT_CHECK_TT CHECK (THANHTIEN > 0)
)
CREATE TABLE MONHOC (
...
CREATE TABLE DIEM (
  ...
  DIEM FLOAT NOT NULL,
  CONSTRAINT DIEM_CHECK CHECK ((DIEM >= 0) AND (DIEM <= 10))
);
...
--> KHONG THE THEM DONG DU LIEU NAY VAO BANG DUOC VI DA SAI DINH DANG NGAY THANG LA YYYY-MM-DD

--CAU 4-----------------------------------------------
INSERT INTO LOP values  ('L004','LL002','L?p 4',10,'K001');
--> KHONG THE THEM DONG DU LIEU VAO BANG VI THUOC TINH SI SO CO RANG BUOC LA SISO>12 MA DONG DU LIEU CO SISO = 10

--CAU 5-----------------------------------------------
DELETE FROM KHOAHOC
WHERE MAKH = 'K001';
--> KHONG THE XOA THUOC TINH 'K001' VI NO LA KHOA CHINH
...
--HOCVIEN_NAM(MAHV,TENHV,SDT,NGAYSINH,DIACHI)
