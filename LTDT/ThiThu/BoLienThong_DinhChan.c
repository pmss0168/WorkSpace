...
int Xuli(Graph *G, int n)
{
    ...
    for (i = 0; i <= n; i++)
    {
        int j;
        int flag = 1;
        for (j = 1; j <= PA[i].size; j++)
        {
            if (element_at(&PA[i], j) % 2 != 0) // chan thi khac 0 le thi bang 0
            {
                flag = 0;
                break;
            }
...
int main(int argc, char const *argv[])
{
    ...
    Tarjan(&G);
    if (Xuli(&G, cntLT))
        printf("Co bo phan lien thong toan dinh chan!!\n");
    else
        printf("Khong co bo phan lien thong toan dinh chan!!\n");
    while (!empty_stack(&check))
    {
        int i = top(&check);
        pop(&check);
        int j;
